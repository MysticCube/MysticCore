package me.thomaszoord.mysticcube.commands.admin.npcs;

import me.thomaszoord.mysticcube.commands.impl.ZCommandExecutor;
import me.thomaszoord.mysticcube.listeners.npcs.NPCInteractListener;
import net.citizensnpcs.api.CitizensAPI;
import net.citizensnpcs.api.npc.NPC;
import net.citizensnpcs.trait.Gravity;
import net.citizensnpcs.trait.HologramTrait;
import net.citizensnpcs.trait.SkinTrait;
import org.bukkit.Location;
import org.bukkit.entity.EntityType;
import org.bukkit.entity.Player;

public class AdminSpawnerCommand extends ZCommandExecutor {
    public AdminSpawnerCommand() {
        super("spawner", "mc.admin", "Set the Spawner npc for teleport");
    }

    @Override
    protected void onCommand(Player p, String[] args) {
        criarNpc(p.getLocation());
        p.sendMessage("§a[Core] NPC Spawner successful created!");
    }

    private void criarNpc(Location l){
        NPC npc = CitizensAPI.getNPCRegistry()
                .createNPC(EntityType.PLAYER, "§8[NPC] Spawner", l);

        npc.spawn(l);
        npc.data().setPersistent(NPC.Metadata.NAMEPLATE_VISIBLE, false);

        Gravity gravityTrait = npc.getOrAddTrait(Gravity.class);
        gravityTrait.setEnabled(true);

        SkinTrait skinTrait = npc.getOrAddTrait(SkinTrait.class);
        String signature = "";
        String value = "";

        skinTrait.setSkinPersistent("skull", signature, value);

        HologramTrait hologramTrait = npc.getOrAddTrait(HologramTrait.class);

        hologramTrait.setLineHeight(0.25);
        hologramTrait.addLine("§7/spawners");
        hologramTrait.addLine("");
        hologramTrait.addLine("§fDon’t miss this chance and get yours now!");
        hologramTrait.addLine("§fCreate and automatic farm by acquiring spawners.");
        hologramTrait.addLine("");
        hologramTrait.addLine("§8(Click to see more)");

        hologramTrait.addLine("§5§lSPAWNERS");

        if(NPCInteractListener.spawnerNpc != null){
            NPCInteractListener.spawnerNpc.despawn();
        }

        NPCInteractListener.spawnerNpc = npc;
    }
}
