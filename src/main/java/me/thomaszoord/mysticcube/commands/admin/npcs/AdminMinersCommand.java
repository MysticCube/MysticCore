...
public class AdminMinersCommand extends ZCommandExecutor {
    public AdminMinersCommand() {
        super("miners", "mc.admin", "Set the miners npc for teleport");
    }

    @Override
    protected void onCommand(Player p, String[] args) {
        criarNpc(p.getLocation());
        p.sendMessage("§a[Core] NPC Miners successful created!");
    }

    private void criarNpc(Location l){
        NPC npc = CitizensAPI.getNPCRegistry()
                .createNPC(EntityType.PLAYER, "§8[NPC] Miners", l);
        ...
        skinTrait.setSkinPersistent("miners", signature, value);


        HologramTrait hologramTrait = npc.getOrAddTrait(HologramTrait.class);

        hologramTrait.setLineHeight(0.25);
        hologramTrait.addLine("§7/miners");
        hologramTrait.addLine("");
        hologramTrait.addLine("§f&fDon’t miss this chance and get yours now!");
        hologramTrait.addLine("&fCreate an automatic farm by acquiring miners.");
        hologramTrait.addLine("");
        hologramTrait.addLine("§8(Click to see more)");

        hologramTrait.addLine("§e§lMINERS");

        if(NPCInteractListener.minersNPC != null){
            NPCInteractListener.minersNPC.despawn();
        }

        NPCInteractListener.minersNPC = npc;
